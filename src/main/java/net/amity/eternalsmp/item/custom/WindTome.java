...
public class WindTome extends Item implements ProjectileItem {
    private static final int COOLDOWN = 10;

    public WindTome(Item.Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient()) {
            WindChargeEntity windChargeEntity = new WindChargeEntity(user, world, user.getX(), user.getEyeY(), user.getZ());
            windChargeEntity.setVelocity(user, user.getPitch(), user.getYaw(), 0.0F, 1.5F, 1.0F);
            world.spawnEntity(windChargeEntity);
        }

        world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_WIND_CHARGE_THROW, SoundCategory.NEUTRAL, 0.5F, 1.0F);
